---
title: "Informe 3: Descripción del atlas celular"
...
```{r, fig.width=10, fig.height=6}
DimPlot(data, label = F , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_main_def") +
      theme(legend.position = 'right') 
```
...
```{r, fig.width=10, fig.height=6}
DimPlot(data, label = T , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_main_def_numeric") +
      theme(legend.position = 'right') 
```
...
```{r, fig.width=14, fig.height=6}
DimPlot(data, label = F , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_main_def", split.by = "Strain") +
      theme(legend.position = 'right') 
```
...
```{r, fig.width=14, fig.height=6}
DimPlot(data, label = T , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_main_def_numeric", split.by = "Strain") +
      theme(legend.position = 'right') 
```
...
dfdonut %>% dplyr::filter(N > 0) %>% kable()
```
...
main_markers_filt <- dplyr::filter(main_markers_filt, gene %in% grep('^Ig[jkl]', main_markers_filt$gene, invert=T, value=T))
...
functionGO <- function(main_markers){
  ...
  for (i in 1:length(unique(main_markers$cluster))){
    ...
    gene_list_original <- df$gene
    gene_list <- grep('^Ig[jklh]', gene_list_original, invert=T, value=T)
  ...
  dotplot(enriched, label_format = 50, showCategory = 5, color = "pvalue") + 
...
```{r, fig.width=12, fig.height=6}
DimPlot(data, label = F , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_subclusters_def") +
      theme(legend.position = 'right') 
```
...
```{r, fig.width=10, fig.height=6}
DimPlot(data, label = T , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_subclusters_def_numeric") +
      theme(legend.position = 'right') 
```
...
```{r, fig.width=16, fig.height=6}
DimPlot(data, label = F , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_subclusters_def", split.by = "Strain") +
      theme(legend.position = 'right') 
```
...
```{r, fig.width=14, fig.height=6}
DimPlot(data, label = T , repel = T, label.size = 3, pt.size = 0.5, reduction = "umap_final", group.by = "Clusters_subclusters_def_numeric", split.by = "Strain") +
      theme(legend.position = 'right') 
```
...
colnames(dfbarplot_general) <- c("Subcluster", "Main", "N")
dfbarplot_general$Supercluster <- dfbarplot_general$Main
dfbarplot_general$Supercluster[dfbarplot_general$Supercluster %in% c("T cells", "ILC", "NK")] <- "T cells"
dfbarplot_general$Supercluster[dfbarplot_general$Supercluster %in% c("Myeloid cells", "Granulocytes")] <- "Myeloid cells"
dfbarplot_general$Supercluster <- droplevels(dfbarplot_general$Supercluster)
dfbarplot_general <- dplyr::filter(dfbarplot_general, N != 0)
dfbarplot_general <- dfbarplot_general %>% group_by(Supercluster) %>% mutate(Perc= round(N * 100 / sum(N),2))

# Barplot
bctab <- dplyr::filter(dfbarplot_general, Supercluster == "B cells")
p1 <- ggplot(bctab, aes(fill=Subcluster, y=N, x=Supercluster)) + 
    geom_bar(position="fill", stat="identity") +
  theme_minimal()  +
  labs(x = "", y = "Fraction of cells")

tctab <- dplyr::filter(dfbarplot_general, Supercluster == "T cells")
p2 <- ggplot(tctab, aes(fill=Subcluster, y=N, x=Supercluster)) + 
    geom_bar(position="fill", stat="identity") +
  theme_minimal()  +
  labs(x = "", y = "Fraction of cells") 

mctab <- dplyr::filter(dfbarplot_general, Supercluster == "Myeloid cells")
p3 <- ggplot(mctab, aes(fill=Subcluster, y=N, x=Supercluster)) + 
    geom_bar(position="fill", stat="identity") +
  theme_minimal()  +
  labs(x = "", y = "Fraction of cells") 
...
```{r}
pc <- subset(x=data, Clusters_subclusters_def %in% c("B cells (PC-IgA)", "B cells (PC-IgG1)", "B cells (PC-IgG2bc)", "B cells (PC-IgM)", "B cells (PC-IgD)"))
...
```{r, fig.width=14, fig.height=6}
FeaturePlot(mycells, features = c("Ptprc"), reduction = "umap_sub") 
```
